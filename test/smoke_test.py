...
from lib.utils import find_grub_tool
...
def main():
    ...
    try:
        ...
        check_file_contains(
            os.path.join(lpss_mnt, 'lpss.conf'),
            fake_uuid,
            'UUID in lpss.conf',
        )
        ...
        check_file(
            os.path.join(esp_mnt, 'EFI', 'BOOT', 'BOOTX64.EFI'),
            'LPSS BOOTX64.EFI',
        )
        check_file(
        ...
        check_file_contains(
            os.path.join(lpss_mnt, 'lpss.conf'),
            '[testnoinitrd]',
            'entry testnoinitrd added',
        )
        check_file_contains(
            os.path.join(lpss_mnt, 'lpss.conf'),
            'type=root',
            'entry type is root (noinitrd)',
        )
        with open(os.path.join(lpss_mnt, 'lpss.conf')) as f:
            full_conf = f.read()
        start = full_conf.find('[testnoinitrd]')
        if start == -1:
            print("  [FAIL] section [testnoinitrd] not found")
            fail_count += 1
        else:
            ...
            if 'initrd=' in section_text:
                print(
                    "  [FAIL] initrd absent for testnoinitrd: "
                    "'initrd=' found in section",
                )
                fail_count += 1
            else:
                print("  [PASS] initrd absent for testnoinitrd")
                pass_count += 1
        check_file_contains(
            os.path.join(grub_dir, 'grub.cfg'),
            'entry_testnoinitrd',
